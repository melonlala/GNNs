...
class GraphUtils:
    ...
    def get_num_workflows(self, graph_path: str) -> int:
        num_workflows = 0
        for subdir in os.listdir(graph_path):
            if os.path.isdir(os.path.join(graph_path, subdir)):
                if subdir.startswith("round_"):
                    if os.path.exists(os.path.join(graph_path, subdir, "graph.py")):
                        num_workflows += 1
    ...
    def load_graph_from_particular_file(self, workflow_path: str):
        workflow_path = workflow_path.replace("\\", ".").replace("/", ".")
        ...
        try:
            ...
            logger.info(f"Error loading graph for {workflow_path}: {e}")
            raise
