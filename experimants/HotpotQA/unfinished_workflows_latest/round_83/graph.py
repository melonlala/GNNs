...
import metagpt.ext.aflow.scripts.optimized.HotpotQA.workflows.round_83.prompt as prompt_custom
...
class Workflow:
    ...
    async def __call__(self, problem: str):
        ...
        if len(preliminary_response['response']) < 50:  # Check for detail
        ...
        # Ensure correctness within expected criteria
        if not ("ft²" in preliminary_response["response"] or "square feet" in preliminary_response["response"]):
            preliminary_response['response'] += " Adding measurements for clarity."
