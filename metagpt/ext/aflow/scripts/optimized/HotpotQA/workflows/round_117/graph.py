...
import metagpt.ext.aflow.scripts.optimized.HotpotQA.workflows.round_117.prompt as prompt_custom
...
class Workflow:
    ...
    async def __call__(self, problem: str):
        ...
        # Combine context and problem for a richer preliminary answer
        combined_input = f"{problem} Context: {context_response['response']}"
        preliminary_response = await self.custom(input=combined_input, instruction=prompt_custom.REVIEW_PROMPT)
        ...
        multiple_answers = [await self.custom(input=problem + f" Summary: {summary_response['response']}", instruction=prompt_custom.FINAL_PROMPT) for _ in range(3)]
