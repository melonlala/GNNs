...
import copy
import re
from typing import List,Union,Literal, Any, Callable, List, Tuple
import random
import regex
...
def parse_args():
    ...
    parser.add_argument('--num_iterations', type=int, default=50,help="The num of training iterations.")
    parser.add_argument('--domain', type=str, default="math",help="Domain (the same as dataset name), default 'gsm8k'")
    parser.add_argument('--agent_names', nargs='+', type=str, default=['MathSolver'],
                        help='Specify agent names as a list of strings')
    parser.add_argument('--agent_nums', nargs='+', type=int, default=[8],
    ...
    parser.add_argument('--dataset_json', type=str, default=f"{GDesigner_ROOT}/my_datasets/math.jsonl")
...
async def main():
    ...
    result_dir = Path(f"{args.basedir}/{args.domain}/labels/")
    result_dir.mkdir(parents=True, exist_ok=True)
    result_file = result_dir / f"{args.llm_name}_{args.agent_nums}agents_{current_time}.json"
    result_edge_index_dir = Path(f"{args.basedir}/{args.domain}/edge_index/")
    result_edge_index_dir.mkdir(parents=True, exist_ok=True)
    result_edge_index = result_edge_index_dir/ f"{args.llm_name}_{args.agent_nums}agents_{current_time}.json"
    mode = args.mode
    ...
    for i_batch in range(num_batches):
        ...
        for i_record, record in enumerate(current_batch):
            ...
            task = record["problem"]
            answer = extract_math_answer(record["solution"])
        ...
        for task, answer, log_prob, true_answer, edge_index in zip(current_batch, raw_answers, log_probs, answers, edge_index_lst):
            nodes_info = {graph.nodes[node].id: graph.nodes[node].role for node in graph.nodes}
            graph_item = {"task": task['problem'],
                        #   "context": task['context'], 
                          "nodes":nodes_info , 
                          "edge_index":edge_index}
            predict_answer = gsm_get_predict(answer[0])
            is_solved = math_equal(predict_answer, true_answer)
...
def get_kwargs(mode:Union[Literal['DirectAnswer'],Literal['FullConnected'],Literal['Random'],Literal['Chain'],Literal['Debate'],Literal['Layered'],Literal['Star']]
    ...
    def get_profile(N):
        ...
        if N> 4:
            ...
            profiles = random.sample(profile_pool, N)
        return profiles
