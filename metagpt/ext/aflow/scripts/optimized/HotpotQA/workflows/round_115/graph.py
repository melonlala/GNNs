...
import metagpt.ext.aflow.scripts.optimized.HotpotQA.workflows.round_115.prompt as prompt_custom
...
class Workflow:
    ...
    async def __call__(self, problem: str):
        ...
        if "clear" not in validation_response['response'].lower() or "irrelevant" in validation_response['response'].lower():
        ...
        if "not found" in final_review_response['answer'].lower() and len(self.context_history) > 0:
            random_context = random.choice(self.context_history)  # Selecting a random context
            fallback_review_response = await self.custom(input=problem + " Last context was: " + random_context, instruction=prompt_custom.FALLBACK_PROMPT)
