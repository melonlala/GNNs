...
import metagpt.ext.aflow.scripts.optimized.HotpotQA.workflows.round_47.prompt as prompt_custom
...
class Workflow:
    ...
    async def __call__(self, problem: str):
        ...
        # Review the context response for accuracy and relevance
        context_review_response = await self.custom(input=context_response['response'], instruction=prompt_custom.CONTEXT_REVIEW_PROMPT)
        ...
        multiple_answers = [await self.custom(input=problem + f" Context: {context_review_response['response']} Review: {review_response['thought']} Summary: {summary_response['response']}", instruction=prompt_custom.FINAL_PROMPT) for _ in range(3)]
