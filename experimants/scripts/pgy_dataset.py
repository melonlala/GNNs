...
from experiments.scripts.utils.reader.py import JSONLReader
...
def get_dataloader(args,branches):
    ...
    for branch in branches:
        ...
        dataset = CustomGraphDataset(root=f"{args.data_path}/{branch}", jsonl_path=jsonl_path,brach=branch)
...
class CustomGraphDataset(InMemoryDataset):
    def __init__(self, root, jsonl_path,branch, transform=None, pre_transform=None, raw_data_list = None,
                 encoder = None):
        ...
        self.data_list = self.raw_process(raw_data_list)
    ...
    def raw_process(self):
        raw_data_lst = self.raw_data_list
        memory = {}
        ...
        for data in tqdm(raw_data_lst,desc="Getting prompt embedding"):
            ...
            node_attr_embedding = self.encoder.encode(node_attr)
