...
import metagpt.ext.aflow.scripts.optimized.HotpotQA.workflows.round_119.prompt as prompt_custom
...
class Workflow:
    ...
    async def __call__(self, problem: str):
        ...
        # New alternative solutions generation step
        alternative_solutions = await self.custom(input=problem + f" Context: {context_response['response']} Summary: {summary_response['response']}", instruction=prompt_custom.ALTERNATIVE_PROMPT)
