...
import metagpt.ext.aflow.scripts.optimized.MATH.workflows.round_60.prompt as prompt_custom
...
class Workflow:
    ...
    async def __call__(self, problem: str):
        ...
        # If the review is satisfactory, proceed to execute the solution
        if "correct" in review_response['response'].lower():
            solution = await self.programmer(problem=response['response'])
