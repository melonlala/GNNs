...
import metagpt.ext.aflow.scripts.optimized.HotpotQA.workflows.round_64.prompt as prompt_custom
...
class Workflow:
    ...
    async def __call__(self, problem: str):
        ...
        # Perform a secondary review on the summary for accuracy
        secondary_review_response = await self.custom(input=summary_response['response'], instruction=prompt_custom.REVIEW_DETAIL_PROMPT)

        # Generate multiple answers for self-consistency
        multiple_answers = [await self.custom(input=problem + f" Context: {context_response['response']} Review: {review_response['thought']} Summary: {secondary_review_response['response']}", instruction=prompt_custom.FINAL_PROMPT) for _ in range(3)]
