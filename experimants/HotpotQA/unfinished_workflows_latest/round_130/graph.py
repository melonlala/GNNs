...
import metagpt.ext.aflow.scripts.optimized.HotpotQA.workflows.round_130.prompt as prompt_custom
...
class Workflow:
    ...
    async def __call__(self, problem: str):
        ...
        # Self-review each generated answer for accuracy
        reviewed_answers = [await self.answer_generate(input=ans['response']) for ans in multiple_answers]
        
        # An additional review after generating multiple answers
        additional_reviews = [await self.answer_generate(input=ans['answer']) for ans in reviewed_answers]

        # Use self-consistency to select the best solution
        ensemble_response = await self.sc_ensemble(solutions=[ans['answer'] for ans in additional_reviews])
