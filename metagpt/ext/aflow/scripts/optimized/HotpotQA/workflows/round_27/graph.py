...
class Workflow:
    ...
    async def __call__(self, problem: str):
        ...
        ensemble_responses = [await self.custom(input=problem + f" Review: {review_response['thought']}", instruction=prompt_custom.FINAL_PROMPT) for _ in range(3)]
        
        # Finalize the solution using ensemble
        final_solution = await self.sc_ensemble(solutions=[response['response'] for response in ensemble_responses])
