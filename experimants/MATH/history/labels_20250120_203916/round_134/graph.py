...
import metagpt.ext.aflow.scripts.optimized.MATH.workflows.round_134.prompt as prompt_custom
...
class Workflow:
    ...
    async def __call__(self, problem: str):
        ...
        # Add ensemble review step
        ensemble_review_instruction = f"Evaluate the multiple outputs including: solution {solution['response']} and validation {validation_result['output']}."
        ensemble_review_result = await self.custom(input=ensemble_review_instruction, instruction=prompt_custom.REVIEW_PROMPT)

        # Use ensemble to improve the final answer selection with enriched output
        ensemble_result = await self.ensemble(solutions=[solution['response'], validation_result['output'], ensemble_review_result['response']], problem=problem)
