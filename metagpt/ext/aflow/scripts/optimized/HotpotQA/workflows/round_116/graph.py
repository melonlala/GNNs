...
import metagpt.ext.aflow.scripts.optimized.HotpotQA.workflows.round_116.prompt as prompt_custom
...
class Workflow:
    ...
    async def __call__(self, problem: str):
        ...
        decomposition_response = await self.custom(input=problem, instruction=prompt_custom.DECOMPOSITION_PROMPT)
        
        # Generate a detailed context for the problem components
        context_response = await self.custom(input=decomposition_response['response'], instruction=prompt_custom.CONTEXT_PROMPT)
        
        # Analyze the components to ensure comprehensive coverage
        analysis_response = await self.custom(input=context_response['response'], instruction=prompt_custom.ANALYZE_PROMPT)
        ...
        # Review the preliminary response for accuracy
        ...
        multiple_answers = [await self.custom(input=problem + f" Context: {context_response['response']} Analysis: {analysis_response['response']} Review: {review_response['thought']}", instruction=prompt_custom.FINAL_PROMPT) for _ in range(3)]
