...
os.environ["HTTP_PROXY"] = "http://127.0.0.1:7835"
os.environ["HTTPS_PROXY"] = "http://127.0.0.1:7835"
...
def parse_args():
    ...
    parser.add_argument('--dataset_json', type=str, default=f"{GDesigner_ROOT}/datasets/hotpotqa.jsonl")
...
async def main():
    ...
    for i_batch in range(num_batches):
        ...
        # print(edge_index)
        for task, answer,edge_index, log_prob, correct_answer in zip(current_batch, raw_answers, edge_index_lst, log_probs, correct_answers):

            nodes_info = {graph.nodes[node].id: graph.nodes[node].role for node in graph.nodes}
            graph_item = {"task": task['question'],
            ...
            single_loss = -log_prob * score
            loss_list.append(single_loss)
            avy_utility = sum(utilities) / len(utilities)
            updated_item = {
                "Question": task,
                "Attempt answer": answer,
                "Solution": answer,
                "Score": score,
                "Average Utility": avy_utility
            }
...
def get_kwargs(mode:Union[Literal['DirectAnswer'],Literal['FullConnected'],Literal['Random'],Literal['Chain'],Literal['Debate'],Literal['Layered'],Literal['Star'],Literal['Mesh'],
    ...
    def get_profile(N, profile_choice):
        profile_pool = ['Knowlegable Expert',
                        #  'Wiki Searcher',
                         'Critic',
                         'Mathematician',
                         'Psychologist',
