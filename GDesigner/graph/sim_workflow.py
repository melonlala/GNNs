...
class SimWorkflow(ABC):
    def __init__(self,
        ...
        self.agent_names = [agent_name for _ in range(len(workflow["nodes"]))]
        self.decision_method: str = decision_method
        self.node_kwargs =  [{} for _ in self.workflow["nodes"]]
    ...
    def construct_node_embedding(self):
        features = []
    ...
    def init_nodes(self):
        # assert len(self.agent_names) == len(self.workflow["nodes"])
        sim_node_id_lst = list(self.workflow["nodes"].keys())
        sim_node_role_lst = list(self.workflow["nodes"].values())
        for agent_name, sim_node_id, sim_node_role, kwargs in zip(self.agent_names, sim_node_id_lst ,sim_node_role_lst, self.node_kwargs):
        # for sim_node_id, sim_node_role, kwargs in zip(sim_node_id_lst ,sim_node_role_lst, self.node_kwargs):    
        #     kwargs["domain"] = self.domain
        #     kwargs["llm_name"] = self.llm_name
        #     kwargs["id"] = sim_node_id
        #     kwargs["role"] = sim_node_role
        #     agent_instance = AgentRegistry.get('CodeWriting', **kwargs)
        #     self.add_node(agent_instance)
            if agent_name in AgentRegistry.registry:
                ...
                self.add_node(agent_instance)
                # for sim_role in sim_node_role_lst:
                #     if agent_instance.role == sim_role:
                #         agent_instance.id = sim_node_id_lst[sim_node_role_lst.index(sim_role)]
                #         self.add_node(agent_instance)

                #         del sim_node_id_lst[sim_node_role_lst.index(sim_role)]
                #         del sim_node_role_lst[sim_node_role_lst.index(sim_role)]
                #         break
                        # sim_node_id_lst = sim_node_id_lst.remove(sim_node_id_lst[sim_node_role_lst.index(sim_role)])
                        # sim_node_role_lst = sim_node_role_lst.remove(sim_role)
                        # break
    
    def init_edge_index(self):
        for edge in self.workflow["edge_index"]:
            self.nodes[edge[0]].add_successor(self.nodes[edge[1]])
            self.nodes[edge[1]].add_predecessor(self.nodes[edge[0]])
        ...
        for i, node1_id in enumerate(self.nodes):
            for j, node2_id in enumerate(self.nodes):
                if self.nodes[node2_id] in self.nodes[node1_id].spatial_successors:
                    edge_index.append([node1_id,node2_id])
        return edge_index
    ...
    async def arun(self, input: Dict[str, str],
                ...
                in_degree = {node_id: len(node.spatial_predecessors)
                            for node_id, node in self.nodes.items()}
                zero_in_degree_queue = [node_id for node_id,
                                        deg in in_degree.items() if deg == 0]
                ...
                while zero_in_degree_queue:
                    ...
                    while tries < max_tries:
                        try:
                            ...
                            print(
                                f"Error during execution of node {current_node_id}: {e}")
                        tries += 1
...
#             fixed_spatial_masks = [[1 if i != j else 0 for j in range(
#                 len(agent_names))] for i in range(len(agent_names))]
...
#         assert len(fixed_spatial_masks) == len(
#             agent_names)*len(agent_names), "The fixed_spatial_masks doesn't match the number of agents"
#         assert len(fixed_temporal_masks) == len(
#             agent_names)*len(agent_names), "The fixed_temporal_masks doesn't match the number of agents"
...
#         # add potential edges to the self.potential_spatial/temporal_edges
...
#             fixed_spatial_masks, requires_grad=False)  # fixed edge masks

#         init_temporal_logit = torch.log(torch.tensor(initial_temporal_probability / (
#             1 - initial_temporal_probability))) if optimized_temporal else 10.0
...
#             fixed_temporal_masks, requires_grad=False)  # fixed edge masks

            
#     def construct_adj_matrix(self):
#         role_connect: List[Tuple[str, str]
#                            ] = self.prompt_set.get_role_connection()
...
#         if len(final_answers) == 0:
#             final_answers.append("No answer of the decision node")
#         return final_answers, log_probs, edge_index
...
#             prune_num_edges = torch.round(
#                 num_edges*pruning_rate) if torch.round(num_edges*pruning_rate) > 0 else 1
