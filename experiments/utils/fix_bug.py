...
async def main():
    args = parse_args()
    work_flow_dir = Path(f"{GDesigner_ROOT}/large_scale_workflow/composed_workflows/chosen_workflows.json")
    workflow_data = load_result(work_flow_dir)
    # random.shuffle(workflow_data)

    workflow_data = workflow_data[:30]
    # save the chosen workflows to a file
    chosen_workflow_file = Path(f"{GDesigner_ROOT}/large_scale_workflow/composed_workflows/chosen_workflows.json")
    with open(chosen_workflow_file, 'w', encoding='utf-8') as file: 
    ...
    result_dir = Path(f"{GDesigner_ROOT}/simulations/eval")
    ...
    for i, workflow in enumerate(workflow_data):
        if i < 15:
            continue
        # find existing result file in result directory
        result_dir = Path(f"{GDesigner_ROOT}/simulations/eval/combined/")
        for file in result_dir.iterdir():
            if file.name.endswith(f"workflow_{i}.json_combined.json"):
                result_file = file
        exist_data = load_result(result_file)
        dataset = JSONLReader.parse_file(args.dataset_json)
        dataset_names = [item['name'] for item in dataset]
        for item in exist_data:
            if item["Question"]['name'] in dataset_names:
                dataset_names.remove(item["Question"]['name'])
        # reset dataset
        print(f"Number of remaining questions: {len(dataset)} in workflow_{i}")
        dataset = [item for item in dataset if item['name'] in dataset_names]
        if len(dataset) == 0:
            print(f"No more data available in workflow_{i}")
        else:
            print(f"Number of remaining questions: {len(dataset)} in workflow_{i}")
        total_executed = 0
        total_solved = 0
        ...
        num_batches = len(dataset) // args.batch_size + 1
