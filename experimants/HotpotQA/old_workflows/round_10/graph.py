...
import metagpt.ext.aflow.scripts.optimized.HotpotQA.workflows.round_10.prompt as prompt_custom
...
class Workflow:
    ...
    async def __call__(self, problem: str):
        ...
        # Review the answer details for additional context
        review_context = await self.custom(input=answer_details['thought'], instruction="Provide additional context for this thought.")
        
        # Use the custom method to generate the initial response
        initial_solution = await self.custom(input=problem + f" Details: {review_context['response']}", instruction="")
        ...
        review_solution = await self.custom(input=initial_solution['response'], instruction="Review this answer for accuracy. Summary: " + summary_solution['answer'])
