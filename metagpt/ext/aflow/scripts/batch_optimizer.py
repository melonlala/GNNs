...
class Optimizer:
    ...
    async def _optimize_graph(self):
        ...
        len_dataset = await self.get_dataset_len(self.dataset, test=False)
        for i_task in range(len_dataset):
            if self.round == 1:
                directory = self.graph_utils.create_round_directory(graph_path, self.round, i_task)
                # Load graph using graph_utils
                self.graph = self.graph_utils.load_graph(self.round, graph_path)
                avg_score = await self.evaluation_utils.evaluate_graph(self, directory, validation_n, data, initial=True, va_list=[i_task])
                

            # Create a loop until the generated graph meets the check conditions
            while True:
                directory = self.graph_utils.create_round_directory(graph_path, self.round + 1, i_task)
            ...
            avg_score = await self.evaluation_utils.evaluate_graph(self, directory, validation_n, data, initial=False, va_list = [i_task])
