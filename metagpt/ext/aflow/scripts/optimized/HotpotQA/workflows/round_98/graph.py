...
import metagpt.ext.aflow.scripts.optimized.HotpotQA.workflows.round_98.prompt as prompt_custom
...
class Workflow:
    ...
    async def __call__(self, problem: str):
        ...
        candidate_solutions = []
        for i in range(3):  # Generate 3 candidate answers
            candidate = await self.custom(input=problem + f" Thought process: {review['response']}", instruction="")
            candidate_solutions.append(candidate['response'])
