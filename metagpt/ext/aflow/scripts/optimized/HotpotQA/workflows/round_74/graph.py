...
import metagpt.ext.aflow.scripts.optimized.HotpotQA.workflows.round_74.prompt as prompt_custom
...
class Workflow:
    ...
    async def __call__(self, problem: str):
        ...
        # Generate a summary based on the context provided
        summary_response = await self.custom(input=context_response['response'], instruction=prompt_custom.SUMMARY_PROMPT)

        # Generate multiple answers for self-consistency
        multiple_answers = [await self.custom(input=problem + f" Context: {context_response['response']} Summary: {summary_response['response']}", instruction=prompt_custom.FINAL_PROMPT) for _ in range(3)]
        ...
        # Perform a review and revise step to ensure accuracy
        review_response = await self.custom(input=final_review_response['answer'], instruction=prompt_custom.REVIEW_PROMPT)
