...
import metagpt.ext.aflow.scripts.optimized.HotpotQA.workflows.round_89.prompt as prompt_custom
...
class Workflow:
    ...
    async def __call__(self, problem: str):
        ...
        multiple_answers = [await self.custom(input=problem + f" Context: {context_response['response']} Analysis: {analysis_response['response']} Review: {review_response['thought']} Summary: {summary_response['response']}", instruction=prompt_custom.FINAL_PROMPT) for _ in range(3)]
