...
import metagpt.ext.aflow.scripts.optimized.HotpotQA.workflows.round_79.prompt as prompt_custom
...
class Workflow:
    def __init__(
        ...
        self.clarity_check = operator.Custom(self.llm)  # New operator for clarity check
    ...
    async def __call__(self, problem: str):
        ...
        context_response = await self.custom(input=problem, instruction=prompt_custom.CONTEXT_PROMPT + f" Follow-up question: What is your main query?")
        ...
        # Clarity check on the review response
        clarity_response = await self.clarity_check(input=review_response['thought'], instruction=prompt_custom.CLARIFICATION_PROMPT)
