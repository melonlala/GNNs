...
import metagpt.ext.aflow.scripts.optimized.HotpotQA.workflows.round_48.prompt as prompt_custom
...
class Workflow:
    ...
    async def __call__(self, problem: str):
        ...
        # Validate the preliminary response for relevance and format
        validation_response = await self.custom(input=preliminary_response['response'], instruction=prompt_custom.VALIDATION_PROMPT)
        if not validation_response['response'] or not isinstance(validation_response['response'], str):
            return "Preliminary response is not relevant or incorrectly formatted", self.llm.cost_manager.total_cost
