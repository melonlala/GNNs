...
import metagpt.ext.aflow.scripts.optimized.HotpotQA.workflows.round_51.prompt as prompt_custom
...
class Workflow:
    ...
    async def __call__(self, problem: str):
        ...
        # Extract key information from the context response
        key_info_response = await self.custom(input=context_response['response'], instruction=prompt_custom.KEY_INFO_PROMPT)
        
        # Validate the context response for relevance and comprehensiveness
        validation_response = await self.custom(input=key_info_response['response'], instruction=prompt_custom.VALIDATION_PROMPT)
        ...
        # Check if the review response is complete
        if len(review_response['thought']) < 50:  # Arbitrary length check for completeness
        ...
        multiple_answers = [await self.custom(input=problem + f" Context: {context_response['response']} Key Info: {key_info_response['response']} Review: {review_response['thought']} Summary: {summary_response['response']}", instruction=prompt_custom.FINAL_PROMPT) for _ in range(3)]
