...
import metagpt.ext.aflow.scripts.optimized.HotpotQA.workflows.round_4.prompt as prompt_custom
...
class Workflow:
    ...
    async def __call__(self, problem: str):
        ...
        ensemble_responses = [await self.custom(input=problem, instruction=prompt_custom.REVIEW_PROMPT) for _ in range(3)]
        
        # Use self-consistency to select the best solution
        ensemble_solution = await self.sc_ensemble(solutions=[resp['response'] for resp in ensemble_responses])
