...
from experiments.scripts.utils.embedding import get_encoder
from experiments.scripts.utils.model import get_model
...
class BaseBenchmark(ABC):
    ...
    def save_results_to_json(self, results: List[Tuple[Any, ...]], columns: List[str]):
        df = pd.DataFrame(results, columns=columns)
        avg_score = df["score"].mean()
        t_cost = df["cost"].max()
        a_cost = t_cost / len(df) if len(df) > 0 else 0
    ...
    async def evaluate_all_problems(self, data: List[dict], graph: Callable, max_concurrent_tasks: int = 50, 
                                    gnn_optimize: bool = False, model_name: str = None, ckp: str = None):
        ...
        if not gnn_optimize:
            ...
            gnn_model = get_model(model_name, ckp)
            device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
            return await self.gnn_evaluate_problem(data, graph, gnn_model, encoder)

    async def run_evaluation(self, graph: Callable, va_list: List[int], max_concurrent_tasks: int = 50, 
                             gnn_optimize: bool = False, ckp: str = None):
        
        data = await self.load_data(va_list)
        results = await self.evaluate_all_problems(data, graph, max_concurrent_tasks, gnn_optimize, ckp)
        columns = self.get_result_columns()
        # average_score, average_cost, total_cost = self.save_results_to_csv(results, columns)
        average_score, average_cost, total_cost = self.save_results_to_json(results, columns)
