...
import metagpt.ext.aflow.scripts.optimized.HotpotQA.workflows.round_120.prompt as prompt_custom
...
class Workflow: 
    def __init__(self, name: str, llm_config, dataset: DatasetType) -> None: 
        ...
        self.review_generate = operator.Custom(self.llm)  # New operator for final review
    ...
    async def __call__(self, problem: str): 
        ...
        if direct_answer_response['response']: 
            # Preliminary review of the direct answer response
            preliminary_review_response = await self.answer_generate(input=direct_answer_response['response']) 
            if preliminary_review_response['thought'] and "unclear" in preliminary_review_response['thought'].lower():
                ...
                final_review_response = await self.review_generate(input=ensemble_response['response']) 
            ...
            # If the initial response is empty, handle accordingly
            return "Inconclusive", self.llm.cost_manager.total_cost
