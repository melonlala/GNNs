import json
def process_raw_mmlu_aflow(file,output_file):
    data = []
    ...
    for item in data:
        ...
        keys_to_keep = ['label','nodes','edge_index','task','source','model']  
...
from convert_dataset import get_dataloader  
from GDesigner.gnn.predictor import Predictor_ofa,Predictor,Predictor_concat   
import numpy as np
import random
...
# def validate_utility(model, loader, device):
#     workflow_count_dict = {}
#     ground_workflow_dict = {}
...
def main(args):
    ...
    # branches = ['workflow_test',"task_test","workflow_task_test"]
    # workflow_test_loader,task_test_loader,workflow_task_test_loader = get_dataloader(args,branches)
    branches = ['test']
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', type=str, default='/home/yuanshuozhang/GDesigner-3063/workflowbench/dataset/gsm8k_aflow', help='Path to the root data directory')
    parser.add_argument('--llm', type=str, default="ST", help='LLM model name')   
    parser.add_argument('--base_conv',type=str,default='TransformerConv',help='Base convolution layer')
    parser.add_argument('--arch',type=str,default='concat',help='architecture of GNN')
    parser.add_argument('--cross_system', type=str, default=None, help='model path when cross system')
