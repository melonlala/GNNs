...
import metagpt.ext.aflow.scripts.optimized.HotpotQA.workflows.round_80.prompt as prompt_custom
...
class Workflow:
    ...
    async def __call__(self, problem: str):
        ...
        # Implementing majority voting for the final answer selection
        final_review_response = await self.answer_generate(input=ensemble_response['response'])
        validation_response = await self.answer_generate(input=final_review_response['answer'])
