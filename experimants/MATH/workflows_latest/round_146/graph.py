...
import metagpt.ext.aflow.scripts.optimized.MATH.workflows.round_146.prompt as prompt_custom
...
class Workflow:
    ...
    async def __call__(self, problem: str):
        ...
        # Review the self-ask output to enhance validation
        review_instruction = f"Review the following inputs and identify potential weaknesses: Solution: {solution['response']} Self-ask: {self_ask_result['response']}"
        ...
        validation_instruction = f"Validate and review the following solution: {solution['response']} {self_ask_result['response']} {review_result['response']}"
