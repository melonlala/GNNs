...
import metagpt.ext.aflow.scripts.optimized.MATH.workflows.round_108.prompt as prompt_custom
...
class Workflow:
    ...
    async def __call__(self, problem: str):
        ...
        # Validate the solution for correctness by generating an explanation
        validation_instruction = f"Validate the following solution and explain: {solution['response']}"
        ...
        # Self-review step to compile a rationale for the solution
        review_instruction = f"Review the steps taken to arrive at the solution: {solution['response']}."
        review_result = await self.custom(input=review_instruction, instruction=prompt_custom.REVIEW_PROMPT)

        # Validate the solution using Python code with explanation
        validation_code = f"def validate_solution():\n    # Explanation for validation logic\n    return {self_ask_result['response']}\n"
        ...
        ensemble_result = await self.ensemble(solutions=[solution['response'], validation_result['output'], self_ask_result['response'], review_result['response']], problem=problem)
