...
import metagpt.ext.aflow.scripts.optimized.MATH.workflows.round_148.prompt as prompt_custom
...
class Workflow:
    ...
    async def __call__(self, problem: str):
        ...
        # Self-ask to refine the understanding of the problem
        self_ask_response = await self.custom(input=problem + f" Please give a detailed analysis of this issue.", instruction="")
        
        # Use ScEnsemble to select the best solution
        ensemble_response = await self.ensemble(solutions=solutions, problem=problem)
        
        # Validate the selected solution through a review step with enhanced instruction
        validation_response = await self.custom(input=ensemble_response['response'], instruction="Please review this proposed solution's accuracy and provide suggestions for improvement.")
