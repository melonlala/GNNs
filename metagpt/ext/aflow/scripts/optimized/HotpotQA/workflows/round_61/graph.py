...
import metagpt.ext.aflow.scripts.optimized.HotpotQA.workflows.round_61.prompt as prompt_custom
...
class Workflow:
    ...
    async def __call__(self, problem: str):
        ...
        # Validate the preliminary response for relevance and completeness
        if not preliminary_response['response'] or len(preliminary_response['response']) < 20:  # Check for relevance and minimum length
            # Request clarification for a more detailed response
            clarification_response = await self.custom(input=problem, instruction=prompt_custom.CLARIFICATION_PROMPT)
            return clarification_response['response'], self.llm.cost_manager.total_cost
