...
class XMLReader(Reader):
    def parse(self, file_path: Path) -> str:
        print(f"Reading XML file from {file_path}.")
        with open(file_path, "r") as f:
...
class YAMLReader(Reader):
    def parse(self, file_path: Path, return_str=True) -> Union[str, Any]:
        print(f"Reading YAML file from {file_path}.")
        with open(file_path, "r") as f:
    ...
    def parse(self, file_path: Path) -> str:
        ...
        with open(file_path, "r") as f:
            ...
            text = "".join(BeautifulSoup(
                data, "html.parser").findAll(string=True))
        return text
...
class ZipReader(Reader):
    def parse(self, file_path: Path) -> str:
        ...
        print(f"Reading ZIP file from {file_path}.")
        try:
            file_content = ""
...
class PythonReader(Reader):
    def parse(self, file_path: Path) -> str:
        print(f"Executing and reading Python file from {file_path}.")
        execution_result = ""
        error = ""
        file_content = ""
        try:
...
class FileReader:
    ...
    def read_file(self, file_path: Path, task="describe the file") -> str:
        ...
        print(
            f"Reading file {file_path} using {type(self.reader).__name__}")
        return file_content
